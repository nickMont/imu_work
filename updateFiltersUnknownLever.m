function [ewxvUp,biasState,Pout,Sk,nuj] = updateFiltersUnknownLever(dtt,ewxv,Pin,imuMeas,gpsMeas,biasState,L_ab0)
%get all time stamps. GPS is assumed to be the last measurement in terms of
%time
%Sk and nuj are only used in MMKF

maxLeverPossible=1e7*[1;1;1];  %change to 1e7 to ignore
...
Qimu=diag([10^-7.5*ones(3,1); 10^-9*ones(3,1)]);
...
if numImuMeas>1
    ...
    Ak=[zer3 eye3 zer3 zer3 zer3 zer3
        ...
        zeros(9,18)];
    Fk=eye(18)+dtt*Ak;
    zk=[exvCF(4:6)-gpsMeas{1}(2:4)];  %pose meas in local frame
    Rk=.05*eye(3);
    Hk=[eye3 zer3 RR*L1b zer3 zer3 RR];
%     zk=[exvCF(1:6)-gpsMeas{1}(2:7)];  %pose+att
%     Rk=.05*eye(6);
%     Hk=[zer3 zer3 eye3 zer3 zer3 zer3
%         eye3 zer3 -RR*L1b zer3 zer3 RR];
...
%     % Best performance with standard filter:
%     ewxvUp=[exvCF(1:3);imuMeas{numImuMeas}(2:4);exvCF(4:9)];

    % Gamma modification
    Rhat = RR*(eye(3)+hatmat(biasState(7:9)));
    e2=calculate_euler_from_R(Rhat);
    ewxvUp=[e2;imuMeas{numImuMeas}(2:4);exvCF(4:9)];
